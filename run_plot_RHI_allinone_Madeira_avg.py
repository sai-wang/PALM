...
import RHI_plot_config_madeira_avg as vc
...
def main():
    ...
    while True:
        ...
        dflidone = get_lidar_data(vc.where_lid, name_lid, vc, header_flag)
...
#   pdb.set_trace()
    # dflid.to_csv(vc.outpath + '\\raw_data.csv', index=False)
    
    # Unfold the elevation range to 0-180 for better plotting    
    dflid['elevation'] = dflid['elevation'].where(dflid['azimuth']<180, 180 - dflid['elevation'])
...
def get_lidar_data(fp, fn, vc, header_flag):
    ...
    if vc.SWITCH_HDF and os.path.isfile( hdfpath ):
        ...
        for f in Path(fp).rglob(fn):
            ...
            # Filter outliers
            dfone = dfone[dfone.cnr>=-28]
            dfone = dfone[dfone.elevation>=2.6]
            dfone = dfone[dfone.elevation<=3.5]
            dfone = dfone[dfone.azimuth>=50.0]

            
            if header_flag ==0:
                dfone.to_csv(vc.outpath + '_rwy05data_clouds.csv.gz', mode='a', columns=['radial_wind_speed','elevation'], index=True, header=True)
                header_flag = 1
            else:
                dfone.to_csv(vc.outpath + '_rwy05data_clouds.csv.gz', mode='a', columns=['radial_wind_speed','elevation'], index=True, header=False)
...
def plot_low_scan(toplot, sProp, clim, sDate, vc):
    ...
    if len(toplot)>0:
        ...
        toplot['elevation'] = np.radians(round(toplot['elevation']))


        if 'scanID' in toplot:
            for sID in toplot['scanID'].unique():
                dfplot = toplot[toplot['scanID']==sID]
                plot_rhi_loop(dfplot, sProp, clim, sDate, vc, sID)
        else:
            plot_rhi_loop(toplot, sProp, clim, sDate, vc, vc.scan_ID)


def plot_rhi_loop(dfplot, sProp, clim, sDate, vc, sID):
            ...
            for s in newscan:
                ...
                try:
                    bpivot = thisscan.reset_index().pivot_table(index='elevation',columns='range',values=sProp)
                except ValueError:
                    plotthis = thisscan.reset_index().set_index(['elevation','range'])
                    plotthis = plotthis[plotthis.index.duplicated(keep='first')]
                    bpivot = plotthis.reset_index().pivot(index='elevation',columns='range',values=sProp)
                    pdb.set_trace()
                ...
                cp =  plt.pcolormesh(bpivot.index.values, bpivot.columns.values,\
                        plotarr, edgecolors='none', vmin=clim[0], vmax=clim[1],cmap='bwr')
                cb = plt.colorbar(cp)
                cb.set_label(sProp)
...
#               ax.set_thetamax(298)
                ...
                # save plot
                plt.savefig(os.path.join(vc.outpath, sDate + '_' + thisscan.index[0][0].strftime('%y%m%d%H%M%S') + '_' + nameadd + '_rhi.png'), dpi=200)
                plt.close()
                n=s

            if sProp!='tele':
                try:
                    filenameadd = nameadd + '_rhi.png'
                    outnameadd = nameadd + '_rhi.gif'
                    # create_gif(filenameadd, sDate, outnameadd, vc)
                except RuntimeError:
                    print('... RuntimeError while creating RHI gif')
                except IndexError:
                    print('... IndexError while creating RHI gif')
