...
!-- Sample for user-defined output
    ...
    REAL(wp), DIMENSION(:,:,:), ALLOCATABLE ::  v2      !< user defined array
    REAL(wp), DIMENSION(:,:,:), ALLOCATABLE ::  v2_av   !< user defined array
    REAL(wp), DIMENSION(:,:,:), ALLOCATABLE ::  w2      !< user defined array
    REAL(wp), DIMENSION(:,:,:), ALLOCATABLE ::  w2_av   !< user defined array
...
!-- Sample for user-defined output
    ...
    ALLOCATE( v2(nzb:nzt+1,nysg:nyng,nxlg:nxrg) )
    ALLOCATE( w2(nzb:nzt+1,nysg:nyng,nxlg:nxrg) )
...
!--    Uncomment and extend the following lines, if necessary
       ...
       CASE ( 'v2', 'v2_xy', 'v2_xz', 'v2_yz' )
          found  = .TRUE.
          grid_x = 'x'
          grid_y = 'yv'
          grid_z = 'zu'
          
       CASE ( 'w2', 'w2_xy', 'w2_xz', 'w2_yz' )
          found  = .TRUE.
          grid_x = 'x'
          grid_y = 'y'
          grid_z = 'zw'    
...
!--       Sample for user-defined output:
          ...
          CASE ( 'v2' )
             IF ( .NOT. ALLOCATED( v2_av ) )  THEN
                ALLOCATE( v2_av(nzb:nzt+1,nysg:nyng,nxlg:nxrg) )
             ENDIF
             v2_av = 0.0_wp
             
          CASE ( 'w2' )
             IF ( .NOT. ALLOCATED( w2_av ) )  THEN
                ALLOCATE( w2_av(nzb:nzt+1,nysg:nyng,nxlg:nxrg) )
             ENDIF
             w2_av = 0.0_wp
...
!--       Sample for user-defined output:
          ...
          CASE ( 'v2' )
             IF ( ALLOCATED( v2_av ) )  THEN
                DO  i = nxlg, nxrg
                   DO  j = nysg, nyng
                      DO  k = nzb, nzt+1
                         v2_av(k,j,i) = v2_av(k,j,i) + v2(k,j,i)
                      ENDDO
                   ENDDO
                ENDDO
             ENDIF
          ...
          CASE ( 'w2' )
             IF ( ALLOCATED( w2_av ) )  THEN
                DO  i = nxlg, nxrg
                   DO  j = nysg, nyng
                      DO  k = nzb, nzt+1
                         w2_av(k,j,i) = w2_av(k,j,i) + w2(k,j,i)
                      ENDDO
                   ENDDO
                ENDDO
             ENDIF                         
          ...
          CASE ( 'v2' )
             IF ( ALLOCATED( v2_av ) )  THEN
                DO  i = nxlg, nxrg
                   DO  j = nysg, nyng
                      DO  k = nzb, nzt+1
                         v2_av(k,j,i) = v2_av(k,j,i) / REAL( average_count_3d, KIND=wp )
                      ENDDO
                   ENDDO
                ENDDO
             ENDIF
          ...
          CASE ( 'w2' )
             IF ( ALLOCATED( w2_av ) )  THEN
                DO  i = nxlg, nxrg
                   DO  j = nysg, nyng
                      DO  k = nzb, nzt+1
                         w2_av(k,j,i) = w2_av(k,j,i) / REAL( average_count_3d, KIND=wp )
                      ENDDO
                   ENDDO
                ENDDO
             ENDIF
...
!--    Sample for user-defined output:
       ...
       CASE ( 'v2_xy', 'v2_xz', 'v2_yz' )
          IF ( av == 0 )  THEN
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   DO  k = nzb_do, nzt_do
                      local_pf(i,j,k) = v2(k,j,i)
                   ENDDO
                ENDDO
             ENDDO
          ELSE
             IF ( .NOT. ALLOCATED( v2_av ) )  THEN
                ALLOCATE( v2_av(nzb:nzt+1,nysg:nyng,nxlg:nxrg) )
                v2_av = REAL( fill_value, KIND = wp )
             ENDIF
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   DO  k = nzb_do, nzt_do
                      local_pf(i,j,k) = v2_av(k,j,i)
                   ENDDO
                ENDDO
             ENDDO
          ENDIF
       ...
       CASE ( 'w2_xy', 'w2_xz', 'w2_yz' )
          IF ( av == 0 )  THEN
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   DO  k = nzb_do, nzt_do
                      local_pf(i,j,k) = w2(k,j,i)
                   ENDDO
                ENDDO
             ENDDO
          ELSE
             IF ( .NOT. ALLOCATED( w2_av ) )  THEN
                ALLOCATE( w2_av(nzb:nzt+1,nysg:nyng,nxlg:nxrg) )
                w2_av = REAL( fill_value, KIND = wp )
             ENDIF
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   DO  k = nzb_do, nzt_do
                      local_pf(i,j,k) = w2_av(k,j,i)
                   ENDDO
                ENDDO
             ENDDO
          ENDIF
